...
(pause-protocol)
```
Pauses all state-changing operations. Callable by owner or any admin.
...
```
Submits an oracle's score report for a provider in the current epoch. Each oracle may submit exactly one report per provider per epoch. All scores must be in the range 0-10000 (basis points). The `evidence-hash` is a 32-byte hash of off-chain evidence data. The provider must be active and the oracle must be active and registered.
...
```
Aggregates score reports from the supplied oracle IDs for the given provider and epoch. Requires a minimum of 3 valid reports. Computes the composite score as: `(accuracy * 40 + security * 40 + liveness * 20) / 100`. Writes the result to the provider record. Callable by owner or admin.
...
3. Verify the project structure:
   ...
   clarinet check
   ```
   This validates the contract syntax and configuration.
...
1. Ensure you have a funded testnet wallet. You can request testnet STX from the [Stacks faucet](https://explorer.hiro.so/sandbox/faucet?chain=testnet).
...
4. Confirm the deployment on the [Stacks Testnet Explorer](https://explorer.hiro.so/?chain=testnet).
